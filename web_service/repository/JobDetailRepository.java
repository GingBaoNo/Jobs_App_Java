...
public interface JobDetailRepository extends JpaRepository<JobDetail, Integer> {
    ...
    // Đã thêm điều kiện lọc công việc hết hạn
    ...
    // Phương thức tìm kiếm công việc mới nhất - Đã thêm điều kiện lọc ngày hết hạn
    @Query("SELECT j FROM JobDetail j WHERE j.trangThaiDuyet = :trangThaiDuyet AND j.trangThaiTinTuyen = :trangThaiTinTuyen " +
           "AND (j.ngayKetThucTuyenDung IS NULL OR j.ngayKetThucTuyenDung >= CURRENT_DATE) " +
    ...
    // Phương thức tìm kiếm top 10 công việc nổi bật theo lượt xem - Đã thêm điều kiện lọc ngày hết hạn
    @Query("SELECT j FROM JobDetail j WHERE j.trangThaiDuyet = :trangThaiDuyet AND j.trangThaiTinTuyen = :trangThaiTinTuyen " +
           "AND (j.ngayKetThucTuyenDung IS NULL OR j.ngayKetThucTuyenDung >= CURRENT_DATE) " +
           "ORDER BY j.luotXem DESC")
    ...
    // Phương thức tìm kiếm công việc theo danh sách ID lĩnh vực - Đã thêm điều kiện lọc ngày hết hạn
    @Query("SELECT j FROM JobDetail j WHERE j.workField.maLinhVuc IN :fieldIds " +
           "AND j.trangThaiDuyet = 'Đã duyệt' AND j.trangThaiTinTuyen = 'Mở' " +
           "AND (j.ngayKetThucTuyenDung IS NULL OR j.ngayKetThucTuyenDung >= CURRENT_DATE)")
